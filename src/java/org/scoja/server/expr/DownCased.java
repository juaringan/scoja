...
public class DownCased extends String2StringFunction {
    
    public DownCased(final StringExpression subexpr) {
        super(subexpr);
    }
    
    protected StringExpression cloneCleanWith(final StringExpression subclon) {
        return new DownCased(subclon);
    }
    
    public QStr eval(final EventContext ectx) {
        final QStr val = super.eval(ectx);
        if (val == null) return null;
        return QStr.unchecked(val.unqualified().toLowerCase());
    }
}
