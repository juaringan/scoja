...
public class TwoDigitsDateHole extends DateHoleSkeleton {

    protected final int key;
    protected final int shift;

    public TwoDigitsDateHole(final String name, final int whichTimestamp,
                             final int key) {
    ...
    public void toFilename(final StringBuffer sb, final EventContext ectx) {
        try {
            DateUtils.append2Digits0(sb, get(ectx,key) + shift);
        } catch (java.io.IOException cannotHappen) {}            
    }
    
    public void writeTo(final PrintWriter out, final EventContext ectx) {
        try {
            DateUtils.append2Digits0(out, get(ectx,key) + shift);
        } catch (java.io.IOException cannotHappen) {}
    }
}
