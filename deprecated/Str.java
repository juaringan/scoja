...
public class Str implements CharSequence {
    ...
    public Str(final byte[] data) {
        this(data, 0);
    }
    
    public Str(final byte[] data, final int start) {
        this(data, start, data.length-start);
    }
    
    public Str(final byte[] data, final int start, final int length) {
        this.data = data;
        this.start = start;
        this.length = length;
    }
    ...
    public CharSequence subSequence(final int start, final int end) {
        return new Str(data, this.start+start, end-start);
    }
